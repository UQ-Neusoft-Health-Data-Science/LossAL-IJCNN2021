...
alpha_const=0.75
output_dir="/home/s4616573/data/bert_ner_output/"+data_name+"_loss_alpha4_"+str(alpha_const)+"_no_exlusion/"
...
log_file=output_dir+'results/'+data_name+'_alpha'+str(alpha_const)+"_"+str(uncertain_threshold)+'_losssum_sumLC_simi_no_exlusion.list_level_neighbor.txt'
handlers = [
...
def train_regression_loss(params):
    ...
    for result in results:
        ...
        unlabelled_loss_list.append(0.25*sample_loss+0.75*params["uncertain_unlabelled"][ucounter])#
   ...
   # # trained_dissi_scores=np.multiply(dissi_arrays,istrained_arrays)
    ...
    if no_neighbors==False :
        simi_test_instances = {u_target_id: [[z, u_target_id, simi_test_scores[u_target_id, z]] for z in
                                             list(set(simi_desired_thres_idx_dict[simi_thres][u_target_id]).difference(set(related_test_data)))
    ...
    if simi_test_num>0 and no_neighbors==False:
        ...
        [simi_test_dict[test_val[0]]["idx"].append(test_val[1]) for test_val in test_values]
        [simi_test_dict[test_val[0]]["simi_values"].append(test_val[2]) for test_val in test_values]
    ...
    for u_target_id in params["unlabelled_idx"]:
        ...
        if len(desired_sim_id) > 0:
            ...
            total_val =total_val#undefined_value if total_val<uncertain_threshold*len_text else total_val#*len_text
        else:
...
if __name__ == '__main__':
    ...
    params = {
        ...
        'simi_thres_id': 0,
        'simi_thres_list': [0.9,0.8,0.7,0.6,0.5],
        # 'simi_thres_list': [ 0.5,  0.4, 0.3,  0.2, 0.1, 0],
